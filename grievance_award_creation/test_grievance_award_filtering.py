...
class FilterGrievanceAwards(TestCase):
    def setUp(self):
        ...
        self.tempCase1 = Case()
        self.tempCase1.lead = self.tempPerson.id
        self.tempCase1.complainant = self.tempPerson
        self.tempCase1.campus = "Saskatoon"
        self.tempCase1.school = "School of Rock"
        self.tempCase1.program = self.program
        self.tempCase1.caseType = 1
        ...
        self.tempCase3 = Case()
        self.tempCase3.lead = self.tempPerson.id
        self.tempCase3.complainant = self.tempPerson
        self.tempCase3.campus = "Saskatoon"
        self.tempCase3.school = "School of Technology"
        self.tempCase3.program = self.program
        self.tempCase3.caseType = 3
    ...
    def test_type_filetering(self):
        response = self.client.get('/api-root/grievance_award/filter', {'type': 'M'})
    ...
    def test_award_amount_filtering(self):
        response = self.client.get('/api-root/grievance_award/filter', {'amount': '50.00'})
    ...
    def test_award_description(self):
        response = self.client.get('/api-root/grievance_award/filter', {'description' : 'es'})
        self.assertEqual(response.json()['count'], 2)
        self.assertEqual(response.json()['results'][0]['id'], self.ga1.id)
        self.assertEqual(response.json()['results'][1]['id'], self.ga3.id)
    ...
    def test_all_fields_in_one_filter(self):
        response = self.client.get('/api-root/grievance_award/filter',
                                   {'min_date': '2015-12-01', 'max_date': '2017-12-01',
                                    'amount': "2500.00", 'description': 'Test', 'type':'M'})
        self.assertEqual(response.json()['count'], 1)
        self.assertEqual(response.json()['results'][0]['id'], self.ga1.id)
