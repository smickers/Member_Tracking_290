        ...
        for i in range(0, sheet.to_records().__len__(), 1):  # loop for each row
            ...
            if valid:  # If the row is valid
                for z in range(0, len(temp), 1):  # loop each column of the row
                    ...
                    if "Date" in str(temp[z]):
                        d = sheet[i, z]
                        if d:
                            temp2[field_name[temp[z]]] = d.strftime('%Y-%m-%d')
                        else:
                            temp2[field_name[temp[z]]] = None
                    elif temp[z] != "Name":   # If the current column does not belong to the 'Name' column
